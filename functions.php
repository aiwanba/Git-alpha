...
function deel_breadcrumbs(){
    ...
    $parents = get_category_parents($category->term_id, true, ' <small>></small> ');
    if (is_wp_error($parents)) {
        $parents = '';
    }
    return '<a title="返回首页" href="' . home_url() . '"><i class="fa fa-home"></i></a> <small>></small> ' . $parents . '<span class="muted">' . get_the_title() . '</span>';
}
...
function git_page_id( $pagephp ) {
    ...
    $row = $wpdb->get_row("SELECT `post_id` FROM `{$wpdb->postmeta}` WHERE `meta_value` = 'pages/{$pagephp}.php'", ARRAY_A);
...
class Git_Tax_Image{
    ...
    public function edit_tax_image_field( $term ){
        ...
        $image = isset($term_meta['tax_image']) ? $term_meta['tax_image'] : '';
        $keywords = isset($term_meta['tax_title']) ? $term_meta['tax_title'] : '';
...
// 修复友情链接小工具显示：隐藏描述和评分，只显示链接名，并双栏排列
function git_fix_links_widget_css() {
    ?>
    <style>
    .widget_links ul {
        font-size: 0;
...
add_action('wp_head', 'git_fix_links_widget_css');
