...
function mod_newcomments($limit, $outpost, $outer) {
	...
	if(false === $comments) {
		...
		$args = array(
			...
			'type'           => 'comment',
			'post_status'    => 'publish',
			'author__not_in' => array( $outer ),
			'post__not_in'   => array( $outpost ),
			'number'         => $limit,
		);
		$comments = get_comments( $args );
		set_transient('comments', $comments, 60*60*2);
		//缓存2小时
	}
	$output='';
	foreach ($comments as $comment) {
		$output.= '<li><a target="_blank" href="' . get_permalink($comment->comment_post_ID) . '#comment-' . $comment->comment_ID . '" title="' . $comment->post_title . '上的评论">' . str_replace(' src=', ' data-original=', get_avatar($comment->comment_author_email, $size = '36', deel_avatar_default())) . ' <div class="muted"><i>' . strip_tags($comment->comment_author) . '</i>' . timeago($comment->comment_date_gmt) . '说：' . str_replace(' src=', ' data-original=', convert_smilies(strip_tags($comment->comment_content))) . '</div></a></li>';
	}
	echo $output;
}
;
...
class git_rec extends WP_Widget {
		...
		for ($x=1; $x<=5; $x++) {
			?>
			  <p>
			      <label>
			          第<?php echo $x;
			?>组文字：<input style="width:200px;" id="<?php
  ...
  echo $instance['atitle'.$x]; ?>" />
			  ...
			  <p>
			      <label>
			          第<?php echo $x;
			?>组链接：<input style="width:200px;" id="<?php
  ...
  echo $instance['alink'.$x]; ?>" />
			  ...
			  <p>
			      <label>
			          第<?php echo $x;
			?>组样式：<select style="width:200px;" id="<?php
  echo $this->get_field_id('aclass'.$x); ?>" name="<?php
  echo $this->get_field_name('aclass'.$x); ?>" >
			              <option value="aclass01" <?php
			  selected('aclass01', $instance['aclass'.$x]);
			?>>黑色</option>
			              <option value="aclass02" <?php
			  selected('aclass02', $instance['aclass'.$x]);
			?>>蓝色</option>
			              <option value="aclass03" <?php
			  selected('aclass03', $instance['aclass'.$x]);
			?>>红色</option>
			              <option value="aclass04" <?php
			  selected('aclass04', $instance['aclass'.$x]);
			?>>黄色</option>
			              <option value="aclass05" <?php
			  selected('aclass05', $instance['aclass'.$x]);
			?>>绿色</option>
			          </select>
			      </label>
			  </p><hr />
		...
		for ($x=1; $x<=4; $x++) {
			?>
			        <p>
			            <label>
			                幻灯<?php echo $x;
			?>图片：<span class="git_tip">360×200</span>
			                <input id="<?php
		...
		for ($x=1; $x<=4; $x++) {
			?>
			        <p>
			            <label>
			                幻灯<?php echo $x;
			?>图片：<span class="git_tip">360×149</span>
			                <input id="<?php
...
class git_social extends WP_Widget {
	...
	function widget($args, $instance) {
		extract($args);
		$title = (!empty($instance['title'])) ? $instance['title'] : '社交按钮';
...
class git_tongji extends WP_Widget {
	...
	function efan_get_blogstat($establish_time 
	/*, $instance */
	) {
		global $wpdb;
