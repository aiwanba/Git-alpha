...
function git_update_options() {
	...
	foreach ($theme_options as $panel) {
		foreach ($panel as $option) {
			...
			$type = $option['type'];
			if ( !$id ) continue;
			$default_options[$id] = isset( $option['std'] ) ? $option['std'] : '';
...
function git_get_option($id, $returnDefault = false) {
	...
	$arr = $returnDefault ? $default_options : $current_options;
	return isset($arr[$id]) ? stripslashes($arr[$id]) : '';
}
...
?>
		<label>
		<span class="description"><?php echo isset($option['before']) ? $option['before'] : ''; ?></span>
